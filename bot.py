...
SCHEDULED_MESSAGES = [
    {'text': 'First message', 'post_at': (
        datetime.now() + timedelta(seconds=20)).timestamp(), 'channel': '#un-test-para-una-vacante'},
    {'text': 'Second Message!', 'post_at': (
        datetime.now() + timedelta(seconds=30)).timestamp(), 'channel': '#un-test-para-una-vacante'}
]
...
if __name__ == "__main__":
    ...
    ids = list_scheduled_messages('#un-test-para-una-vacante')
    delete_scheduled_messages(ids, '#un-test-para-una-vacante')
    app.run(debug=True)
